...
namespace HomeWork
{
    ...
    {
        public static int RepeatCount(this string str, char element)
        {
            int count = 0;
            for (int i = 0; i < str.Length; i++)
            {
                if (str[i].Equals(element)) count++;
            }
            return count;
        }
    }
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //1- Debtors with emails in "rhyta.com" or "dayrep.com" domains

            /*List<Debtor> list = new List<Debtor>();
            foreach (var item in debtors)
            {
                if (item.Email.Contains("rhyta.com") || item.Email.Contains("dayrep.com"))
                {
                    list.Add(item);
                }
            }
            ...
            //2- Print debtors between the ages of 26 and 36

            /*List<Debtor> list = new List<Debtor>();
            foreach (var item in debtors)
            {
                if (item.BirthDay.Year >= 1986 && item.BirthDay.Year <= 1996)
                {
                    list.Add(item);
                }
            }
            ...
            //3- Print debtors not more than 5000 debt
            ...
            //4- Print debtors whose fullname is more than 18 characters and whose phone number has 2 or more "7" numbers

            /*List<Debtor> list = new List<Debtor>();
            foreach (var item in debtors)
            {
                if (item.FullName.Length >= 18)
                {
                    int oldCount = item.Phone.Length;
                    item.Phone = item.Phone.Replace("7", "");
                    int newCount = item.Phone.Length;
            ...
            //5- Print debtors born in the winter

            /*List<Debtor> list = new List<Debtor>();
            foreach (var item in debtors)
            {
                if (item.BirthDay.Month == 12 || item.BirthDay.Month == 1 || item.BirthDay.Month == 2)
                {
                    list.Add(item);
                }
            }
            ...
            //6- Print debtors whose debt is greater than the average debt of the total debt and sort it first according to the name and then according to the reduction of the amount

            /*List<Debtor> list = new List<Debtor>();
            list = debtors.FindAll(debtor =>
            {
                var totalDebt = debtors.Select(deb => deb.Debt).Sum();
                return totalDebt / debtors.Count < debtor.Debt;
            });
            //Normal
            list.ForEach(debtor => Console.WriteLine(debtor));
            //SortedByDebt
            list = list.OrderBy(debtor => debtor.Debt).ToList();
            list.ForEach(debtor => Console.WriteLine(debtor));
            //SortedByName
            list = list.OrderBy(debtor =>
            {
                var name = debtor.FullName.Split();
                return name[2];
            }).ToList();
            list.ForEach(debtor => Console.WriteLine(debtor));*/


            //7- Print only the last name, age and total debt amount of debtors who do not have "8" number on their phone number

            /*List<Debtor> list = new List<Debtor>();
            list = debtors.FindAll(debtor => debtor.Phone.RepeatCount('8').Equals(0));
            list.ForEach(debtor =>
            {
                var name = debtor.FullName.Split();
                var age = DateTime.Now.Year - debtor.BirthDay.Year;
                Console.WriteLine($"{name[2]} {age} {debtor.Debt}");
            });*/


            //8- Print a list of debtors with at least 3 identical letters in their name and surname and sort them alphabetically

            /*List<Debtor> list = new List<Debtor>();
            list = debtors.FindAll(debtor =>
            {
                var name = string.Concat(debtor.FullName.Where(c => !char.IsWhiteSpace(c)));
                name = debtor.FullName.ToLower();
                foreach (var item in name)
                    return name.RepeatCount(item) >= 3;
                return default;
            });
            list = list.OrderBy(debtor => debtor.FullName).ToList();
            list.ForEach(debtor => Console.WriteLine(debtor));*/


            //9- Print the year of birth of debtors born in the same year at most
            ...
            {
                int count = 0;
                foreach (var item2 in debtors)
                {
                    if (item.BirthDay.Year == item2.BirthDay.Year)
                        count++;
                }
                if (max <= count)
                {
            ...
            //10- Print a list of the 5 debtors with the most debt

            /*List<Debtor> list = new List<Debtor>();
            var temp = debtors.OrderBy(debtor => debtor.Debt).ToList();
            for (int i = temp.Count - 1; i >= temp.Count - 5; i--)
            {
                list.Add(temp[i]);
            }
            list.ForEach(debtor => Console.WriteLine(debtor));*/
            ...
            //11- Print the sum of the debts of all debtors

            /*Console.WriteLine(debtors.Select(debtor => debtor.Debt).Sum());*/


            //12- Print a list of debtors who witnessed World War II

            /*List<Debtor> list = new List<Debtor>();
            list = debtors.FindAll(debtor => debtor.BirthDay.Year < 1945);
            list.ForEach(debtor => Console.WriteLine(debtor));*/


            //13- Print a list of debtors from whom we can write the word "smile" from the letters in his name and surname

            /*List<Debtor> list = new List<Debtor>();
            list = debtors.FindAll(debtor =>
            {
                var tempName = debtor.FullName.ToLower();
                return tempName.Contains("s") && tempName.Contains("m") && tempName.Contains("i") && tempName.Contains("l") && tempName.Contains("e");
            });
            list.ForEach(debtor => Console.WriteLine(debtor));*/
